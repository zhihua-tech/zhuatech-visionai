 ...
 */
@SpringBootTest @AutoConfigureMockMvc class VisionAiApiIntegrationTests {
     ...
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanReadShopfloorDashboard()throws Exception{mvc.perform(get("/api/shopfloor/dashboard").header("Authorization","Bearer "+operatorToken)).andExpect(status().isOk()).andExpect(jsonPath("$.success").value(true)).andExpect(jsonPath("$.data.metrics[0].label").value("视觉工位"));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
     ...
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanInspectVisionResult()throws Exception{mvc.perform(post("/api/ai/vision/inspect").header("Authorization","Bearer "+operatorToken).contentType(MediaType.APPLICATION_JSON).content("{\"inspectionNo\":\"VI-001\",\"modelVersion\":\"surface-v3\",\"modelConfidence\":0.97,\"totalDefectCount\":3,\"criticalDefectCount\":1,\"defectAreaRate\":0.018}")).andExpect(status().isOk()).andExpect(jsonPath("$.data.decision").value("BLOCK")).andExpect(jsonPath("$.data.routeTo").value("隔离区"));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
